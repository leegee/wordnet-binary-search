...
import { Wordnet, IndexEntry, Sense, Pointer } from './wordnet';

Wordnet.logger = log4js.getLogger();
Wordnet.logger.level = 'trace';
...
describe('Wordnet', () => {
    xdescribe('X', () => {
        ...
        describe('Index', () => {
            ...
            before(() => {
                verbIndexEntry = Wordnet.getIndexEntry('import', 'v');
            });
        ...
        describe('DataFile', () => {
            it('finds line by synset offset', async () => {
                const line: string = Wordnet.dataFiles.v.getLineBySynsetOffset(2346409);
        ...
        describe('Sense', () => {
            it('from line found by synset offset', async () => {
                const line: string = Wordnet.dataFiles.v.getLineBySynsetOffset(2346409);
                const sense: Sense = Sense.fromLine(line);
        ...
        describe('forms', () => {
            it('finds all forms of  "excuse"', () => {
                const indexEntries = Wordnet.getIndexEntries('excuse');
        ...
        describe('antonyms', () => {
            const testSense = (sense) => {
                const antonym: Sense[] = sense['antonym'];
                if (antonym !== null) {
            ...
            it('finds the opposite of verb "import"', () => {
                const indexEntry = Wordnet.getIndexEntry('import', 'v');
            ...
            it('finds the opposites of all forms of  "import"', () => {
                Wordnet.getIndexEntries('import').forEach(indexEntry => {
    ...
    describe('hypernym', () => {
        it('finds the hypernym of a verb', () => {
            const indexEntry = Wordnet.getIndexEntry('fool', 'v');
        ...
        //     const indexEntry = Wordnet.getIndexEntries('fool');
        //     expect(indexEntry.senses).to.have.length(7);
        //     expect(indexEntry.senses[0].word).to.equal('fool');
        //     expect(indexEntry.senses[0].pCnt).to.equal(4);
        //     expect(indexEntry.senses[0].pointers).to.have.length(4);
        //     expect(indexEntry.senses[0].pointers[0].pos).to.equal('v');
